  ...
  return (
    <Box style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column', height: '100%' }}>
      ...
      <form onSubmit={handleSubmit((form) => handleRegister(form))} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <Button type="submit" variant="contained" style={{ margin: 20, background: theme.primary}}>Créer son compte</Button>
        <Link href="/login" style={{color: theme.primary, textDecorationColor: theme.primary}}>Se connecter</Link>
      </form>
    </Box>
  )
}
